...
(function () {
  ...
  // ---- Input (WASD / flechas) ----
  var keys = {};
  var pointerControl = { active: false, pointerId: null, startX: 0, startY: 0, x: 0, y: 0, fireUntil: 0 };

  function clamp(v, min, max) { return Math.max(min, Math.min(max, v)); }
  function beginPointerControl(e) {
    pointerControl.active = true;
    pointerControl.pointerId = e.pointerId;
    pointerControl.startX = e.clientX;
    pointerControl.startY = e.clientY;
    pointerControl.x = 0;
    pointerControl.y = 0;
    if (renderer.domElement.setPointerCapture) renderer.domElement.setPointerCapture(e.pointerId);
  }
  function movePointerControl(e) {
    if (!pointerControl.active || e.pointerId !== pointerControl.pointerId) return;
    var dx = e.clientX - pointerControl.startX, dy = e.clientY - pointerControl.startY;
    pointerControl.x = clamp(dx / 82, -1, 1);
    pointerControl.y = clamp(-dy / 82, -1, 1);
    // Arrastrar controla la nave y mantiene el fuego activo mientras el dedo/mouse se mueve.
    if (Math.abs(dx) > 3 || Math.abs(dy) > 3) pointerControl.fireUntil = simTime + 0.22;
    e.preventDefault();
  }
  function endPointerControl(e) {
    if (e.pointerId !== pointerControl.pointerId) return;
    pointerControl.active = false;
    pointerControl.pointerId = null;
    pointerControl.x = 0;
    pointerControl.y = 0;
  }
  window.addEventListener('pointermove', movePointerControl, { passive: false });
  window.addEventListener('pointerup', endPointerControl, true);
  window.addEventListener('pointercancel', endPointerControl, true);
  ...
  function updateShip(dt) {
    ...
    if (pointerControl.active) { ax += pointerControl.x; ay += pointerControl.y; }
  ...
  function fireBullets(dt) {
    fire.cd -= dt;
    if (state === 'play' && (keys.Space || (pointerControl.active && simTime < pointerControl.fireUntil)) && fire.cd <= 0) {
  ...
  // Tocar o clickear inicia/reinicia; arrastrar con touch o mouse mueve y dispara.
  window.addEventListener('pointerdown', function (e) {
    if (e.target === audioBtn || e.button !== 0) return;
    if (!audioEngine.active) { audioEngine.start(); syncAudioLabel(); }
    if (state === 'menu' || state === 'crashed') {
      startGame();
      return;
    }
    if (state === 'play') beginPointerControl(e);
  });
  syncAudioLabel();
  ...
  function restart() {
    ...
    shieldBubble.visible = false;
    powerTimer = 8;
    spawnTimer = 0.5; fire.cd = 0; pointerControl.fireUntil = 0;
  ...
  function showMenu() {
    ...
    msg.innerHTML = '<div id="crashBox"><div class="t1">SPACESHIPS</div>' +
      ...
      '<div class="t3">WASD / FLECHAS para volar · ESPACIO para disparar<span class="mobile"><br>ARRASTRA para volar y disparar</span></div>' +
  ...
  window.__game.pointerControl = pointerControl;
  window.__game.debugPointerDrag = function (x, y) { pointerControl.active = true; pointerControl.x = clamp(x, -1, 1); pointerControl.y = clamp(y, -1, 1); pointerControl.fireUntil = simTime + 0.22; };
  window.__game.debugPointerRelease = function () { pointerControl.active = false; pointerControl.x = 0; pointerControl.y = 0; };
